...
[p1, p2, p3, R, S, T, P, n, rounds] = initialize();
...
for n1 = nmin:nmax
    ...
    for n2 = nmin:nmax-n1
        ...
        if n(idx(3)) == 0
            ...
            sortedS = sortedS(1:2);
            % If the second least pay-off strategy has 0 players, no move is
            % possible, stay in the same state
            if n(idx(2))==0
               fprintf('Rule 00: Two strategies with zero players\n');
               fprintf('SA=%.2f, SB=%.2f, SC=%.2f \n',SA, SB, SC);
               [allcurrentstates,allnextstates,allprobs] = remaintosamestate(n,allcurrentstates,allnextstates,allprobs);

            % Here, there are two strategies left with non 0 players
            else
               % If the first one has a larger pay-off than the second one, transfer from the second one to the first  
               if sortedS(1) > sortedS(2)
                   fprintf('Rule 01: Two strategies with non zero players and different pay-offs\n');
                   fprintf('SA=%.2f, SB=%.2f, SC=%.2f \n',SA, SB, SC);
                   [allcurrentstates,allnextstates,allprobs] = transferfromXtoY(2, 1, 1,n,idx,allcurrentstates,allnextstates,allprobs);
               % Here, the two strategies left have the same pay-off. Transfer from either one to the other one with possibility 1 
               else
                   fprintf('Rule 03: Two strategies with non zero players and same pay-offs\n');
                   fprintf('SA=%.2f, SB=%.2f, SC=%.2f \n',SA, SB, SC);
                   [allcurrentstates,allnextstates,allprobs] = transferfromXtoY(1, 2, 0.5,n,idx,allcurrentstates,allnextstates,allprobs);
                   [allcurrentstates,allnextstates,allprobs] = transferfromXtoY(2, 1, 0.5,n,idx,allcurrentstates,allnextstates,allprobs);
               end    
            end
        else
            ...
            if sortedS(1) > sortedS(2) && sortedS(2) > sortedS(3)
                fprintf('Rule 04: Three strategies with non zero players and different pay-offs\n');
                fprintf('SA=%.2f, SB=%.2f, SC=%.2f \n',SA, SB, SC);
                [allcurrentstates,allnextstates,allprobs] = transferfromXtoY(3, 1, 1,n,idx,allcurrentstates,allnextstates,allprobs);
            
            % or the first two have the same pay-off
            elseif sortedS(1) == sortedS(2) && sortedS(2) > sortedS(3)
                fprintf('Rule 05: Three strategies with non zero players, the two higher pay-offs are the same\n');
                fprintf('SA=%.2f, SB=%.2f, SC=%.2f \n',SA, SB, SC);
                [allcurrentstates,allnextstates,allprobs] = transferfromXtoY(3, 1, 0.5,n,idx,allcurrentstates,allnextstates,allprobs);
                [allcurrentstates,allnextstates,allprobs] = transferfromXtoY(3, 2, 0.5,n,idx,allcurrentstates,allnextstates,allprobs);
            % or the last two have the same pay-off
            elseif sortedS(1) > sortedS(2) && sortedS(2) == sortedS(3)
                fprintf('Rule 06: Three strategies with non zero players, the two lower pay-offs are the same\n');
                fprintf('SA=%.2f, SB=%.2f, SC=%.2f \n',SA, SB, SC);
                [allcurrentstates,allnextstates,allprobs] = transferfromXtoY(2, 1, 0.5,n,idx,allcurrentstates,allnextstates,allprobs);
                [allcurrentstates,allnextstates,allprobs] = transferfromXtoY(3, 1, 0.5,n,idx,allcurrentstates,allnextstates,allprobs);
            ...
            elseif sortedS(1) == sortedS(2) && sortedS(2) == sortedS(3)
                fprintf('Rule 07: Three strategies with non zero players all having the same pay-off\n');
                fprintf('SA=%.2f, SB=%.2f, SC=%.2f \n',SA, SB, SC);
                [allcurrentstates,allnextstates,allprobs] = transferfromXtoY(1, 2, 1/6,n,idx,allcurrentstates,allnextstates,allprobs);
                [allcurrentstates,allnextstates,allprobs] = transferfromXtoY(1, 3, 1/6,n,idx,allcurrentstates,allnextstates,allprobs);
                [allcurrentstates,allnextstates,allprobs] = transferfromXtoY(2, 1, 1/6,n,idx,allcurrentstates,allnextstates,allprobs);
                [allcurrentstates,allnextstates,allprobs] = transferfromXtoY(2, 3, 1/6,n,idx,allcurrentstates,allnextstates,allprobs);
                [allcurrentstates,allnextstates,allprobs] = transferfromXtoY(3, 1, 1/6,n,idx,allcurrentstates,allnextstates,allprobs);
                [allcurrentstates,allnextstates,allprobs] = transferfromXtoY(3, 2, 1/6,n,idx,allcurrentstates,allnextstates,allprobs);
            end
        end
...
title("k-step transition matrix M^k heatmap")
...
function [allcurrentstates,allnextstates,allprobs] = remaintosamestate(n,allcurrentstates,allnextstates,allprobs)
    currentstate = "("+num2str(n(1))+","+num2str(n(2))+","+num2str(n(3))+")";
    nextstate = "("+num2str(n(1))+","+num2str(n(2))+","+num2str(n(3))+")";
    probability = 1;
    allcurrentstates=[allcurrentstates;currentstate];
